...
export class FollowerComponent implements OnInit{
    ...
    ngOnInit(){
        console.log('follower.component is load!!');
        this.actualPage();
    }
    ...
    getFollows(userId, page){
        this.myFollowService.getFollower(this.token,userId,page).subscribe(
            response =>{
                if(!response){
                    this.status = 'error';
    ...
    unFolloUser(followed){
        this.myFollowService.deleteFollow(this.token,followed).subscribe(
            response =>{
                var search = this.follower.indexOf(followed);
                if(search != -1){
                    this.follower.splice(search,1);
                }
            },
            error => {
                var errorMessage = <any>error;
